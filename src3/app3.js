...
function partTex(kind,wu,wt,noLabel,lsh){
  var k=KIND[kind], DW=512, DH=256;        /* 絵をかくときの、決まった目もり */
  /* 実際の画素数は、部位の縦横の比に合わせて配りなおす。
     「カマ」のように細長い部位だと、縦がスカスカになって文字が滝のように滲むため。
     画素の総数は今までどおり（512×256＝2の17乗）で、2のべき乗のまま分け直す。 */
  var ww0=Math.max(0.25,wu||1), hh0=Math.max(0.25,wt||1);
  var e=Math.round(Math.log(ww0/hh0)/Math.LN2);
  if(e%2===0) e+=(ww0>=hh0?1:-1);           /* 足して17になるよう、奇数にそろえる */
  e=Math.max(-7,Math.min(7,e));
  var W=Math.pow(2,(17+e)/2), H=Math.pow(2,(17-e)/2);
  var c=cv(W,H,function(x){
    x.setTransform(W/DW,0,0,H/DH,0,0);      /* 以下は 512×256 の感覚で描ける */
    var base=(k.tone==="skin")?"#ffffff":hx(TONE[k.tone]||0xdddddd);
    x.fillStyle=base; x.fillRect(0,0,DW,DH);
    if(k.tone!=="skin"&&k.tone!=="silver"&&k.tone!=="eyeball"&&k.tone!=="dark") grainOn(x,DW,DH);
    if(kind==="medama"){
      x.beginPath(); x.arc(DW*0.5,DH*0.52,66,0,6.3); x.fillStyle="#f7f7f5"; x.fill();
      x.lineWidth=7; x.strokeStyle="rgba(60,60,60,.55)"; x.stroke();
      x.beginPath(); x.arc(DW*0.5,DH*0.52,38,0,6.3); x.fillStyle="#20262b"; x.fill();
      x.beginPath(); x.arc(DW*0.5+13,DH*0.52-13,12,0,6.3); x.fillStyle="#fff"; x.fill();
      return;
    }
    if(k.ribs){
      ...
      for(var rb=0;rb<11;rb++){
        ...
        x.moveTo(bx,DH*0.46); x.bezierCurveTo(bx+16,DH*0.66,bx+12,DH*0.86,bx-10,DH*0.99); x.stroke();
        x.lineWidth=11; x.beginPath();
        x.moveTo(bx,DH*0.44); x.bezierCurveTo(bx+14,DH*0.28,bx+10,DH*0.14,bx-8,DH*0.03); x.stroke();
      }
      for(var sg=0;sg<13;sg++){
        var sx=20+sg*38;
        x.fillStyle="#FFFDF6"; x.beginPath();
        x.ellipse(sx,DH*0.45,17,13,0,0,6.3); x.fill();
  ...
  tx.anisotropy=Math.min(16,renderer.capabilities.getMaxAnisotropy()||4);   /* 斜め見のボケ対策 */
  return tx;
}
